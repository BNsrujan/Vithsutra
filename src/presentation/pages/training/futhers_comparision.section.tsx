  ...
  return (
    <section className="py-12 px-6 w-full bg-gray-50">
      <div className="max-w-[1400px] mx-auto">
        <h2 className={`${text.Sectiontexthead} mb-6 text-center`}>Model Comparison</h2>
        <div className="overflow-x-auto border rounded-lg">
          <table className="min-w-full border-collapse ">
            <thead>
              <tr className="bg-gray-100 text-gray-800">
                <th className={`${text.cardBodytext} text-left p-3 border-r`}>Feature</th>
                {Object.keys(plans).map((plan, idx) => (
                  <th key={idx} className={`${text.cardBodytextlight} p-3 text-center border-r`}>{plan}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {features.map((feature, idx) => (
                <tr key={idx} className="border-t bg-white group group-hover:bg-gray-50  ">
                  <td className={` ${text.cardBodytext} p-3  border-r text-gray-700 group-hover:text-black`}>{feature}</td>
